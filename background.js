...
class EmailMindBackground {
    ...
    async setupAlarms() {
        ...
        const summaryTime = settings['summary-time'] || CONFIG.DEFAULT_SUMMARY_TIME;
        ...
        try {
            ...
            switch (request.action) {
                ...
                case 'summarizeThread':
                    response = await this.summarizeThread(request.threadId);
                    break;
                
                case 'summarizeEmail':
                    response = await this.summarizeEmail(request.messageId);
                    break;
    ...
    async authenticate() {
        try {
            ...
            // Send token to backend for validation/storage
            await this.syncWithBackend(token, profile);
    ...
    async syncWithBackend(token, profile) {
        try {
            const response = await fetch(`${CONFIG.API_BASE_URL}/auth/sync`, {
                method: 'POST',
                headers: {
            ...
            // Don't fail authentication if backend sync fails
        }
    }
    ...
    async disconnect() {
        try {
            ...
            // Notify backend
            try {
                await fetch(`${CONFIG.API_BASE_URL}/auth/disconnect`, {
                    method: 'POST',
                    headers: {
    ...
    async getEmailSummary() {
        try {
            ...
            // Process emails with backend AI
            const summary = await this.processEmailsWithBackend(emails, persona);
            
            // Store summary data
            await this.storeSummaryData(summary.emails, summary.stats);
    ...
    async fetchEmails() {
        const response = await fetch(
            `${CONFIG.GMAIL_API_BASE}/users/me/messages?q=in:inbox&maxResults=${CONFIG.MAX_EMAILS_PER_SUMMARY}`,
            {
                headers: {
                    'Authorization': `Bearer ${this.authToken}`
                }
        ...
        const emailPromises = messages.map(async (msg) => {
            const msgResponse = await fetch(
                `${CONFIG.GMAIL_API_BASE}/users/me/messages/${msg.id}`,
                {
                    headers: {
                        'Authorization': `Bearer ${this.authToken}`
                    }
    ...
    async processEmailsWithBackend(emails, persona) {
        try {
            const response = await fetch(`${CONFIG.API_BASE_URL}${CONFIG.ENDPOINTS.SUMMARIZE}`, {
                ...
                body: JSON.stringify({
                    emails: emails.map(email => this.parseGmailMessage(email)),
                    persona: persona,
                    maxEmails: CONFIG.MAX_EMAILS_PER_SUMMARY
                })
            });
            ...
            return this.processEmailsLocally(emails, persona);
        }
    }

    async processEmailsLocally(emails, persona) {
        // Fallback local processing when backend is unavailable
        const processedEmails = [];

        for (const message of emails.slice(0, 10)) {
            try {
                const emailData = this.parseGmailMessage(message);
                
                if (this.shouldIncludeEmail(emailData, persona)) {
                    const summary = this.generateFallbackSummary(emailData);
                ...
                console.error('Error processing email locally:', error);
            }
        }

        const stats = this.generateStats(processedEmails);
        return { emails: processedEmails, stats };
    }
    ...
    async handleEmailAction(action, emailId) {
        try {
            ...
            if (action === 'archive') {
                const response = await fetch(
                    `${CONFIG.GMAIL_API_BASE}/users/me/messages/${emailId}/modify`,
                    {
                        method: 'POST',
                        headers: {
                            'Authorization': `Bearer ${this.authToken}`,
    ...
    async summarizeThread(threadId) {
        try {
            const response = await fetch(`${CONFIG.API_BASE_URL}/emails/summarize-thread`, {
                method: 'POST',
                headers: {
                    'Content-Type': 'application/json',
                    'Authorization': `Bearer ${this.authToken}`
            ...
            return { success: false, error: 'Failed to summarize thread' };
        } catch (error) {
    ...
    async summarizeEmail(messageId) {
        try {
            const response = await fetch(`${CONFIG.API_BASE_URL}/emails/summarize-email`, {
                method: 'POST',
                headers: {
                    'Content-Type': 'application/json',
                    'Authorization': `Bearer ${this.authToken}`
            ...
            return { success: false, error: 'Failed to summarize email' };
        } catch (error) {
    ...
    async getUserProfile() {
        try {
            const response = await fetch(
                `${CONFIG.GOOGLE_API_BASE}/userinfo`,
                {
                    headers: {
                        'Authorization': `Bearer ${this.authToken}`
                    }
    ...
    async verifyToken() {
        try {
            const response = await fetch(
                `${CONFIG.GOOGLE_API_BASE}/tokeninfo?access_token=${this.authToken}`
            );
            return response.ok;
        } catch (error) {
            return false;
    ...
    async storeAuthData(token, profile) {
        await this.setStorageData({
            ...
            [CONFIG.STORAGE_KEYS.USER_PROFILE]: profile,
            authTime: Date.now()
        });
    }
    ...
    async storeSummaryData(emails, stats) {
        await this.setStorageData({
            [CONFIG.STORAGE_KEYS.LAST_SUMMARY]: {
                emails,
                stats,
                timestamp: Date.now()
            }
    ...
    async loadStoredData() {
        ...
        this.authToken = data[CONFIG.STORAGE_KEYS.AUTH_TOKEN];
        this.userProfile = data[CONFIG.STORAGE_KEYS.USER_PROFILE];
        this.lastSummaryTime = data.lastSummaryTime;
    }

    scheduleNextSummary() {
        chrome.storage.sync.get(['summary-time'], (result) => {
            const summaryTime = result['summary-time'] || CONFIG.DEFAULT_SUMMARY_TIME;
